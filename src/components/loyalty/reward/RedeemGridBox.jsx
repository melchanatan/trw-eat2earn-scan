...
const RedeemGridBox = () => {
  ...
  const fetchCoupon = async () => {
    ...
    if (response.status == 200) {
      const sorted = data.sort((a, b) => a.expDate - b.expDate);
      const filtered = sorted.filter(
        (a) =>
          a.quantity > 0 &&
          new Date(Number(a.expDate)).setUTCHours(0, 0, 0, 0) >=
    ...
    <>
        ...
        >
          <div className="text-center text-primary font-avant">
            ...
            <p className="w-[32ch] opacity-80 mb-6">
              something about eat2earn membership is free for the first 3
              months, but you still need to earn 200 points.
            </p>
          </div>
        </ConfirmationPopup>
      )}
      ...
      >
        ...
        <div className="grid grid-cols-2 ">
          <button
            href=""
            className={`redeem-nav__link
                            ${modeToggle ? "redeem-nav__link--active" : ""}`}
            onClick={() => setModeToggle(true)}
          >
            {modeToggle && (
              <img
                src="/assets/inverted-corner-lighter.svg"
                alt="ds"
                className="redeem-nav__corner--left"
              />
            )}
            <RiShoppingBasketFill className="w-6 h-6" /> Browse Deals
          </button>
          <button
            href=""
            className={`redeem-nav__link
          ...
          >
            {!modeToggle && (
              <img
                src="/assets/inverted-corner-lighter.svg"
                alt="ds"
                className="redeem-nav__corner--right"
              />
            )}
            <FaBoxArchive className="w-5 h-5" /> Coupon
          </button>
        </div>
        <div
