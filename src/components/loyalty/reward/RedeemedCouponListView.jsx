    ...
    <>
          ...
          <div className="flex flex-col text-white font-avant">
            ...
            <div>
              {openScanner ? (
                ...
                <p className="opacity-80">{selectedUserCoupon.detail}</p>
              )}
            </div>
          </div>
        </CouponInfoPopup>
      ...
      {userCoupon <= 0 ? (
        <div className="text-center flex justify-center items-center mt-10 flex-col gap-3 text-gray-600 opacity-60">
          <BiSolidCoupon className="w-20 h-20 shrink-0" />
          <h3 className="font-avant w-[34ch] text-xl font-bold">
            Store you redeemed vouchers here
          </h3>
          <p>
            Vouchers in the wallet will expire if not used and cannot be
            returned or exchanged for points.
          </p>
        </div>
      ) : (
        userCoupon.map((item) => {
          if (item.expDate > new Date().now || true) {
            const exp = new Date(Number(item.expDate)).toLocaleDateString(
              "en-EN",
              {
                year: "numeric",
                month: "long",
...
const CouponInfoPopup = ({
  ...
  return (
    <AnimatePresence>
      <div className="absolute bottom-0 left-0 w-screen h-screen bg-black opacity-50 backdrop-blur-2xl z-40"></div>
      <motion.div
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          ...
          <Button onClick={onCancel} color="outline">
            {cancelText}
          </Button>
        </div>
      </motion.div>
